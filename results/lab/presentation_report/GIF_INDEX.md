...
Файлы по указанным путям появляются после прогона `experiments_lab.run_batch` с флагом **`--render`** для соответствующего batch-конфига (см. `docs/EXPERIMENTS_OVERVIEW.md`). В лёгком checkout репозитория GIF могут отсутствовать.
